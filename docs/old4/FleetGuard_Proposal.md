...
| Model evaluation | `mlflow.genai.evaluate()` on every version, scored with named built-in scorers (`Correctness`, `RetrievalGroundedness`, `Safety`); precision and recall **published on the application's own page** |
...
| Lakebase CDF capture and stream read | 3 s | 6 s |
| DLT micro-batch into fact tables | 5 s | 15 s |
| Dashboard and console read | 2 s | 5 s |
| **Write → visible in analytics** | **~10 s** | **~28 s** |

*Databricks does not publish a latency SLA for Lakebase CDF, so these figures are engineering estimates, not vendor numbers. They are to be measured against the live pipeline during the build, the same way the lead-time backtest is — not asserted in advance.*
...
`wait_after_last_change_seconds` batches rapid successive writes (e.g. an agent working through several signals in one turn) into a single DLT run instead of one run per row; `min_time_between_triggers_seconds` caps run frequency under load. This replaces an assumed continuous-trigger DLT pipeline with a documented, event-driven primitive.
...
**Lakebase CDF retention.** `delta.changeDataFeed.retentionDuration` is set to 30 days on the `lb_<table>_history` tables, comfortably exceeding the DLT refresh interval while bounding storage. `delta.logRetentionDuration` is aligned to the same window.
...
| **Velocity** | **Met — operational path** | Agent write → Lakebase CDF → Delta fact tables in ~10 s average, ~28 s worst case (§8.3). Demonstrable live. |
