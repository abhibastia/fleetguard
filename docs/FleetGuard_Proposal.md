...
**Silver.** Deduplication on `CMPLID`, the true row key — **not** on `ODINO`. This distinction is load-bearing: `ODINO` repeats across the components of a single complaint (NHTSA's own layout says so), and measured against the live corpus it has 1,615,482 distinct values across 2,240,289 rows. Deduplicating on it would silently discard 624,807 legitimate component reports — 27.9% of the corpus, biased against exactly the multi-component defects that matter most. `ODINO` is retained as a complaint-group key. Normalisation of manufacturer, make, and model strings — NHTSA's own changelog documents these shifting across the corpus lifetime. PII in narrative text is **detected and tagged, not deleted**, so it can be masked per-role downstream while remaining available to the embedding pipeline.
...
**Model B — residual match confidence.** The deterministic pass (§7) resolves campaign scope against the roster on make, model, model-year and manufacture window. Model B exists for what that pass cannot settle: manufacturer and model-string variants, vehicles with an unknown manufacture date, and campaigns whose scope text narrows beyond make/model/year. A calibrated gradient-boosted classifier scores those residual cases, with the threshold tuned for recall rather than F1 (§6). It ranks ambiguity; it does not perform the match.

**Measured, and larger than expected.** Only 91 of 163 fleet make/model/year combinations match a recall exactly. NHTSA's spelling routinely differs from vPIC's: the roster holds `F-250` where the recall corpus carries `F-250 SD` (612 rows against 17 for plain `F-250`), alongside `REDUNDANT F-250` and a double-spaced variant. The consequence is concrete — all **2,116 F-250s in the roster match across 22 campaigns purely as model variants, and exact matching returns none of them**. Fleet-wide, variant matches (725,356 rows) outnumber exact matches (263,686) by nearly three to one. `gold_fleet_exposure` records `match_basis` per row so the deterministic guarantee in §7 is scoped to the exact tier, and the residual tier is visibly the thing Model B has to earn.
