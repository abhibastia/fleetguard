...
agent endpoint and AI Search index, redeploy the bundle from `main`, run
`bundle run fleetguard_console`, re-run the agent's smoke test (this time it should reach
`watch_campaign`), and only then call any of this "done." That is genuinely new work, not a
formality — see *Where to go next* below.
...
| resource | state | note |
|---|---|---|
| AI Search `fleetguard-vs` | **DELETED** — re-verified live 2026-09-19 (`list-endpoints`: 0 fleetguard endpoints). A real rebuild was attempted 2026-09-18 (I-105): synced ~7h to 62%, hit a transient embedding-gateway timeout, and the platform's retry restarted from row zero rather than resuming — deleted again rather than pay for a second ~7h attempt | not billing; must be recreated + `complaint_chunk_idx` re-synced before any agent smoke test or demo that touches `search_complaints` (I-101). **Budget for a possible mid-sync restart next attempt (I-105)** — treat ~6.7h as a floor, not a ceiling |
| Databricks App | **STOPPED** — re-verified live 2026-09-19 (`apps get`: `compute_status.state: STOPPED`) | `databricks apps start fleetguard-console`, ~2 min. All three judges hold `CAN_MANAGE` and can start it themselves. **Also not redeployed since PR #6 (2026-09-14)** — starting it today serves pre-`watch_campaign` code, and now also pre-#11-through-#17 (nothing from this session's 6 PRs has shipped to the live App either) |
| Agent endpoint | **NOT_READY** — re-verified live 2026-09-19 (`serving-endpoints list`: `ready: NOT_READY`) | v6, scale-to-zero on. **Also predates `watch_campaign`** — that tool has never been through `agents.deploy()`, and every prompt/tool-reference change from #11's §7.2 write-up is describing code the deployed endpoint doesn't run yet. The 2026-09-18 rebuild attempt never reached Phase B (agent registration), so this is still unchanged |
