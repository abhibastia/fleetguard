 ...
 * interface telling its first lie before the user has even entered.
 */
export function SignIn({ enabled }: { enabled: boolean }) {
  return (
        ...
        {enabled ? (
          <>
            ...
            <p className="footnote" style={{ marginTop: 16 }}>
              Signing in gives you <strong>read access</strong> to the queue, emerging signals and
              evidence. Approving a service campaign additionally requires being on this
              deployment's approver list — signing in proves who you are, not that you may dispatch
              work orders against a fleet.
            </p>
          </>
        ) : (
